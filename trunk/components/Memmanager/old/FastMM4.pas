...
type
  ...
  TMemoryManagerState = packed record
    ...
    {Medium block stats}
    AllocatedMediumBlockCount: system.Cardinal;
    TotalAllocatedMediumBlockSize: system.Cardinal;
    ReservedMediumBlockAddressSpace: system.Cardinal;
    {Large block stats}
    AllocatedLargeBlockCount: system.Cardinal;
    TotalAllocatedLargeBlockSize: system.Cardinal;
    ReservedLargeBlockAddressSpace: system.Cardinal;
  end;
...
{$ifdef EnableMemoryLeakReporting}
  ...
  TRegisteredMemoryLeak = packed record
    LeakAddress: system.Pointer;
    LeakedClass: system.TClass;
    LeakSize: system.Integer;
    LeakCount: system.Integer;
  end;
...
function FastGetMem(ASize: system.Integer): system.Pointer;
function FastFreeMem(APointer: system.Pointer): system.Integer;
function FastReallocMem(APointer: system.Pointer; ANewSize: system.Integer): system.Pointer;
function FastAllocMem(ASize: system.Cardinal): system.Pointer;
{$else}
{The FullDebugMode memory manager functions}
function DebugGetMem(ASize: system.Integer): system.Pointer;
function DebugFreeMem(APointer: system.Pointer): system.Integer;
function DebugReallocMem(APointer: system.Pointer; ANewSize: Integer): system.Pointer;
function DebugAllocMem(ASize: system.Cardinal): system.Pointer;
...
function GetCurrentAllocationGroup: system.Cardinal;
...
procedure PushAllocationGroup(ANewCurrentAllocationGroup: system.Cardinal);
...
{Logs detail about currently allocated memory blocks for the specified range of
 ...
 time.}
procedure LogAllocatedBlocksToFile(AFirstAllocationGroupToLog, ALastAllocationGroupToLog: system.Cardinal);
{$endif}
...
function FastGetHeapStatus: system.THeapStatus;
...
function RegisterExpectedMemoryLeak(ALeakedPointer: system.Pointer): system.boolean; overload;
function RegisterExpectedMemoryLeak(ALeakedObjectClass: system.TClass; ACount: system.Integer = 1): system.boolean; overload;
function RegisterExpectedMemoryLeak(ALeakedBlockSize: system.Integer; ACount: system.Integer = 1): system.boolean; overload;
{Removes expected memory leaks. Returns true on success.}
function UnregisterExpectedMemoryLeak(ALeakedPointer: system.Pointer): system.boolean; overload;
function UnregisterExpectedMemoryLeak(ALeakedObjectClass: system.TClass; ACount: system.Integer = 1): system.boolean; overload;
function UnregisterExpectedMemoryLeak(ALeakedBlockSize: system.Integer; ACount: system.Integer = 1): system.boolean; overload;
...
{$endif}
  ...
  HexTable: array[0..15] of system.char = '0123456789ABCDEF';
...
{$endif}
  ...
  TSmallBlockType = packed record
    ...
    NextSequentialFeedBlockAddress: system.Pointer;
    ...
    MaxSequentialFeedBlockAddress: system.Pointer;
    ...
    MinimumBlockPoolSize: system.Word;
    OptimalBlockPoolSize: system.Word;
...
{$endif}
  ...
  TMediumBlockPoolHeader = packed record
    ...
    {Unused dword}
    Reserved: system.Cardinal;
    {The block size and flags of the first medium block in the block pool}
    FirstMediumBlockSizeAndFlags: system.Cardinal;
  end;
  ...
  TLargeBlockHeader = packed record
    ...
    UserAllocatedSize: system.Cardinal;
    {The size of this block plus the flags}
    BlockSizeAndFlags: system.Cardinal;
  end;
...
{$ifdef EnableMemoryLeakReporting}
  ...
  TExpectedMemoryLeaks = packed record
    ...
    ExpectedLeaks: packed array[0..(ExpectedMemoryLeaksListSize - 20) div system.SizeOf(TExpectedMemoryLeak) - 1] of TExpectedMemoryLeak;
  end;
...
{$endif}
  ...
  SmallBlockPoolHeaderSize = system.SizeOf(TSmallBlockPoolHeader);
  {The size of a medium block pool header}
  MediumBlockPoolHeaderSize = system.SizeOf(TMediumBlockPoolHeader);
  {The size of the header in front of Large blocks}
  LargeBlockHeaderSize = system.SizeOf(TLargeBlockHeader);
{$ifdef FullDebugMode}
...
{$endif}
  ...
  AllocSize2SmallBlockTypeIndX4: packed array[0..(MaximumSmallBlockSize - 1) div SmallBlockGranularity] of system.byte;
  ...
  MediumBlocksLocked: system.boolean;
  {The sequential feed medium block pool.}
  LastSequentiallyFedMediumBlock: system.Pointer;
  MediumSequentialFeedBytesLeft: system.Cardinal;
  {The medium block bins are divided into groups of 32 bins. If a bit
   ...
   blocks.}
  MediumBlockBinGroupBitmap: system.Cardinal;
  {The medium block bins: total of 32 * 32 = 1024 bins of a certain
   minimum size.}
  MediumBlockBinBitmaps: packed array[0..MediumBlockBinGroupCount - 1] of system.Cardinal;
  ...
  LargeBlocksLocked: system.boolean;
...
{$ifdef EnableMemoryLeakReporting}
  ...
  ExpectedMemoryLeaksListLocked: system.Boolean;
{$endif}
  {---------------------Full Debug Mode structures--------------------}
{$ifdef FullDebugMode}
...
{$endif}
  ...
  OldMemoryManager: {$ifndef BDS2006AndUp}system.TMemoryManager{$else}TMemoryManagerEx{$endif};
  {The replacement memory manager}
  NewMemoryManager: {$ifndef BDS2006AndUp}system.TMemoryManager{$else}TMemoryManagerEx{$endif};
{$ifdef DetectMMOperationsAfterUninstall}
  {Invalid handlers to catch MM operations after uninstall}
...
{$endif}
  ...
  FastMMIsInstalled: system.Boolean;
  {Is the MM in place a shared memory manager?}
  IsMemoryManagerOwner: system.Boolean;
...
function LockCmpxchg(CompareVal, NewVal: system.byte; AAddress: system.PByte): system.Byte;
asm
  {On entry:
    al = CompareVal,
    dl = NewVal,
...
{Writes the module filename to the specified buffer and returns the number of
 characters written.}
function AppendModuleFileName(ABuffer: system.PChar): Integer;
var
  LModuleHandle: system.HModule;
begin
...
var
  ...
  {Checks whether the given address is valid address for a VMT entry.}
