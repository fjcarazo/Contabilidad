...
type
  ...
  private
    FPopupEnd: TDateTime;
    FPopupstart: TDateTime;
    procedure SetPopupEnd(const Value: TDateTime);
    procedure SetPopupstart(const Value: TDateTime);
  ...
  public
    ...
    property UserField8: string read FUserField8 write FUserField8;
    property UserField9: string read FUserField9 write FUserField9;
    property WAccountID : Integer  read FWAccountID write FWAccountID;
    property Popupstart : TDateTime  read FPopupstart write SetPopupstart;
    property PopupEnd : TDateTime  read FPopupEnd write SetPopupEnd;
  end;
      ...
      rtCustom:
        ...
        and (Day > trunc(Event.StartTime)) then begin
          ...
          { interval, then we have a recurrence on this day                }
           TempDay  := (Trunc(Day) - Trunc(Event.StartTime)) ;
           if  Event.CustInterval <> 0 then
           result :=  Trunc(TempDay)  mod Event.CustInterval = 0
           else
           result :=  True ;
        end;
    end;
...
procedure TVpContact.SetGender(const Value: String);
begin
  FGender := Value;
end;

procedure TVpEvent.SetPopupEnd(const Value: TDateTime);
begin
  FPopupEnd := Value;
end;

procedure TVpEvent.SetPopupstart(const Value: TDateTime);
begin
  FPopupstart := Value;
end;
