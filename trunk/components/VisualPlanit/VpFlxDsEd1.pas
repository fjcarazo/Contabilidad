...
type
  TfrmFieldMapper = class(TForm)
    ...
    procedure AAutoAddClick(Sender: TObject);
  private
    DSResActive    : Boolean;
...
procedure TfrmFieldMapper.AAutoAddClick(Sender: TObject);
...
begin
  ...
  if MC <> nil then
    begin
    for i := 0 to DatasetFieldLB.Items.Count -1 do
      begin
        DatasetFieldLB.ItemIndex := i ;
        if VPFieldLB.Items.IndexOf(DatasetFieldLB.Items[DatasetFieldLB.ItemIndex]) <> -1 then
          begin
            VPFieldLB.ItemIndex := VPFieldLB.Items.IndexOf(DatasetFieldLB.Items[DatasetFieldLB.ItemIndex]) ;
            FM := TVpFieldMapping(MC.Add);
            FM.DBField := DatasetFieldLB.Items[DatasetFieldLB.ItemIndex];
            FM.VPField := VPFieldLB.Items[VPFieldLB.ItemIndex];
          end;
