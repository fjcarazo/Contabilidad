...
begin
  ...
  try
    ...
    while not ResourceTable.EOF do begin
      ...
      Res.Active := ResourceTable.FieldByName('ResourceActive').AsInteger = 1;
    ...
    with EventsTable do begin
      ...
      while not EventsTable.EOF do begin
        ...
        if Event <> nil then begin
          ...
          Event.AllDayEvent := FieldByName('AllDayEvent').AsInteger=1;
          Event.AlarmSet := FieldByName('AlarmSet').AsInteger=1;
  ...
  if (Resource <> nil) then
    with TasksTable do begin
      ...
      while not EOF do begin
        ...
        Task.Complete := FieldByName('Complete').AsInteger=1;
      ...
      for J := pred(Resource.Schedule.EventCount) downto 0 do begin
        ...
        if Event.Changed then begin
          ...
          try
            ...
            EventsTable.FieldByName('AllDayEvent').AsInteger := ord(Event.AllDayEvent);
            EventsTable.FieldByName('AlarmSet').AsInteger := ord(Event.AlarmSet);
      ...
      for I := pred(Resource.Tasks.Count) downto 0 do begin
        ...
        if Task.Changed then begin
          ...
          try
            ...
            TasksTable.FieldByName('Complete').AsInteger := ord(Task.Complete);
        ...
        if (Res <> nil) and Res.Changed then begin
          with ResourceTable do begin
            ...
            try
              ...
              FieldByName('ResourceActive').AsInteger := ord(Res.Active);
...
{  if Resource = nil then
  ...
  with ResourceTable do begin
    ...
    if Locate('ResourceID', ResourceID, []) then begin
      ...
      Resource.Active := FieldByName('ResourceActive').AsInteger=1;
...
begin
  if aUseDateTime then
    aTable.Filter := Format('ResourceID = %d '
      ...
      [aResourceID,
       QuotedStr(FormatDateTime('yyyy-mm-dd hh:ss:nn', aStartDateTime)),
       QuotedStr(FormatDateTime('yyyy-mm-dd hh:ss:nn', aEndDateTime)),
       QuotedStr(FormatDateTime('yyyy-mm-dd hh:ss:nn', aStartDateTime))])
  else
