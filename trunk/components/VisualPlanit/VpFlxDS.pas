...
uses
  ...
  VpBase, VpData, VpSR, VpDBDS, VpBaseDS, VpException,VpSRVars;
    ...
    if ResourceTable.Active then begin
      ...
      with ResourceTable do begin
        ...
        while not EOF do begin
          ...
          if FN <> '' then
            Res.Active := ResourceTable.FieldByName(FN).AsInteger=1;
      ...
      while not EOF do begin
        ...
        if (FN1 <> '') and (FN2 <> '') and (FN3 <> '') then begin
          ...
          if Event <> nil then begin
            ...
            if (FN1 <> '') then
              Event.AllDayEvent := FieldByName(FN1).AsInteger=1;

            FN1 := GetFieldName(FEventMappings, 'AlarmSet');
            if (FN1 <> '') then
              Event.AlarmSet := FieldByName(FN1).AsInteger=1;
           ...
           FN1 := GetFieldName(FEventMappings, 'WAccountID');
             if FN1 <> '' then
           Event.WAccountID := EventsTable.FieldByName(FN1).AsInteger ;
    ...
    with FContactsDataSrc.DataSet do begin
      ...
      while not EOF do begin
        ...
        if Contact <> nil then begin
          ...
          FN := GetFieldName(FContactMappings, 'Gender');
          if FN <> '' then
            Contact.Gender := FieldByName(FN).AsString;
      ...
      with FTasksDataSrc.DataSet do begin
        ...
        while not EOF do begin
          ...
          if Task <> nil then begin
            ...
            if FN <> '' then
              Task.Complete := FieldByName(FN).AsInteger=1;
    ...
    with FResourceDataSrc.DataSet do begin
      ...
      if FN <> '' then begin
        if Locate(FN, ResourceID, []) then begin
          ...
          if FN <> '' then
            Resource.Active := FieldByName(FN).AsInteger=1;
            ...
            if Event.Changed then begin
              ...
              try
                ...
                if Event.RecordID <> -1 then
                  if EventsTable.FieldByName(FN).CanModify then
                ...
                if FN <> '' then
                  EventsTable.FieldByName(FN).AsInteger := ord(Event.AllDayEvent);

                FN := GetFieldName(FEventMappings, 'AlarmSet');
                if FN <> '' then
                  EventsTable.FieldByName(FN).AsInteger := ord(Event.AlarmSet);
                ...
                FN := GetFieldName(FEventMappings, 'WAccountID');
                if FN <> '' then
                  EventsTable.FieldByName(FN).AsInteger := Event.WAccountID;
      ...
      if Contact.Changed then begin
        ...
        try
          ...
          if Contact.RecordID > -1 then
           if ContactsTable.FieldByName(FN).CanModify then
          ...
          FN := GetFieldName(FContactMappings, 'Gender');
          if FN <> '' then
            ContactsTable.FieldByName(FN).AsString := Contact.Gender;
        ...
        if Task.Changed then begin
          ...
          { RecordID with a bogus value }
            if Task.RecordID > -1 then
             if  FieldByName(FN).CanModify then
            ...
            FN := GetFieldName(FTaskMappings, 'Complete');
            if FN <> '' then
              FieldByName(FN).AsInteger := ord(Task.Complete);
          ...
          if (Res <> nil) and Res.Changed then begin
            with ResourceTable do begin
              ...
              try
                ...
                FN := GetFieldName(FResourceMappings, 'ResourceActive');
                if FN <> '' then
                  FieldByName(FN).AsInteger := ord(Res.Active);
