...
Added special days function for osFinancials.}
...
type
  ...
  TVpOnGetSpecialDate = procedure (Sender : TObject;
                                ADate  : TDateTime;
                                var IsSpecialDate  : Boolean ;var DayText : String;var SpecialColor : Integer ) of object;
  ...
  private
    FTheRenderDate: TDateTime;
    FOnGetSpecialDate: TVpOnGetSpecialDate;
    FNoDragDrop: Boolean;
    procedure SetTheRenderDate(const Value: TDateTime);
    procedure SetOnGetSpecialDate(const Value: TVpOnGetSpecialDate);
    procedure SetNoDragDrop(const Value: Boolean);
  protected{ private }
  ...
  public
    ...
    property TheRenderDate : TDateTime  read FTheRenderDate write SetTheRenderDate;
    property NoDragDrop: Boolean  read FNoDragDrop write SetNoDragDrop;
  published
    property Align;
    property Anchors;
    ...
    property OnGetSpecialDate : TVpOnGetSpecialDate  read FOnGetSpecialDate write SetOnGetSpecialDate;
    property OnClick;
...
/////// Debug Code /////////
  ...
  try
    ...
    { a line in pixels. }
    if (dvLineMatrix[Col, StartLine].Rec.Bottom -
          dvLineMatrix[Col, StartLine].Rec.Top) <> 0 then
        ...
        if not DisplayOnly then begin
          ...
          then begin
            ...
            TPSFillRect (RenderCanvas, Angle, RenderIn, LineRect);
             ...
             if assigned(FOnGetSpecialDate) then
             begin
                FOnGetSpecialDate(self,Trunc(ColDate) + dvLineMatrix[Col, I + StartLine].Time ,IsSpecialDay,SpecialDayText,SpecialColor) ;
             end ;

             if IsSpecialDay then begin
                              { this is a special day }
                              RenderCanvas.Brush.Color := SpecialColor;
                              TPSFillRect (RenderCanvas, Angle, RenderIn, LineRect);
                              TPSTextOut(RenderCanvas, Angle, RenderIn, LineRect.Left +2 ,LineRect.Bottom - 20,SpecialDayText);
               end else
...
(*****************************************************************************)
procedure TVpDayView.SetTheRenderDate(const Value: TDateTime);
begin
  FTheRenderDate := Value;
end;

procedure TVpDayView.SetOnGetSpecialDate(const Value: TVpOnGetSpecialDate);
begin
  FOnGetSpecialDate := Value;
end;

procedure TVpDayView.SetNoDragDrop(const Value: Boolean);
begin
  FNoDragDrop := Value;
end;
