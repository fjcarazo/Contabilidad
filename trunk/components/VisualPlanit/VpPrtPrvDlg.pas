...
uses
  ...
  Buttons,
  VpPrtFmtCBox,
  Printers, ImgList, ComCtrls, ToolWin, ActnList,VpSRVars;
...
type
  ...
  TfrmPrintPreview = class (TForm)
    ...
    cbend: TComboBox;
      ...
      Shift: TShiftState);
    procedure cbendChange(Sender: TObject);
...
begin
  ...
  try
    ...
    EditForm.cbend.ItemIndex := ord(ControlLink.Printer.DayEnd);
    EditForm.cbstart.ItemIndex := ord(ControlLink.Printer.DayStart);
...
procedure TfrmPrintPreview.cbendChange(Sender: TObject);
begin
  VpPrintPreview1.ControlLink.Printer.DayEnd := TVpHours(cbend.ItemIndex);
  VpPrintPreview1.ForceUpdate ;
end;

procedure TfrmPrintPreview.cbstartChange(Sender: TObject);
begin
 VpPrintPreview1.ControlLink.Printer.DayStart := TVpHours(cbstart.ItemIndex);
 VpPrintPreview1.ForceUpdate ;
end;
