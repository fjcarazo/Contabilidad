//***********************************************************
//               IEDownload ver 1.00 (Oct. 15, 2001)        *
//                                                          *
//                       For Delphi 5 & 6                   *
//                     Freeware Component                   *
//                            by                            *
...
uses
  dialogs, ActiveX, Registry, Contnrs, Extctrls, wininet, Windows, UrlMon, SysUtils, Classes;
...
//  grfBINDF := Info.BindInfoValue;
  grfBINDF := BINDF_ASYNCHRONOUS or BINDF_ASYNCSTORAGE or BINDF_PULLDATA;
  with BindInfo do begin
    ...
    begin
      ...
      FDataSize := Length(Info.PostData);
...
begin
  ...
  begin
    bs.info := Info;
    bs.FSender := TIEDownload(Info.Sender);
    bs.Url := Info.Url;
    //binding := nil;
    Succeeded(bs.download);
  end;
end;
...
procedure Register;
begin
  RegisterComponents('Samples', [TIEDownload]);
end;
...
procedure TIEDownload.ClearAll;
begin
  Try
   if bs<>nil then
     bs.ClearAll;
