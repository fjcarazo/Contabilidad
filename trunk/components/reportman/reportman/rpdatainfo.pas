...
{$IFDEF USEUNIDAC}
 {$DEFINE USENEWLINK}
 UniProvider, InterBaseUniProvider, DBAccess, Uni,
{$ENDIF}
...
{$ENDIF}
{$IFDEF USEUNIDAC}
   FZInternalDatabase:TUniConnection;
   FZConnection:TUniConnection;
{$ENDIF}
...
{$ENDIF}
{$IFDEF USEUNIDAC}
   property ZConnection:TUniConnection read FZConnection
    write FZConnection;
{$ENDIF}
{$IFDEF USEZEOS}
...
{$ENDIF}
{$IFNDEF FPC}
procedure CombineAddDataset(client:TJvMemoryData;data:TDataset;group:boolean);
{$ENDIF}
{$ENDIF}
...
{$IFDEF USEZEOS}
var
 GENZConnection:TZConnection;
{$ENDIF}
{$IFDEF USEUNIDAC}
var
 GENZConnection:TUniConnection;
...
{$IFDEF USEUNIDAC}
procedure  AssignParamValuesZ(ZQuery:TUniQuery;Dataset:TDataset);
var
 i:integer;
 afield:TField;
begin
...
function  EqualParamValuesZ(ZQuery:TUniQuery;Dataset:TDataset):Boolean;
var
 i:integer;
 afield:TField;
 qvalue:Variant;
 ...
 try
  ...
  {$IFDEF USEUNIDAC}
   if Not Assigned(FZConnection) then
       begin
        if  self.Alias = 'SETOFBOOKS' then
           FZConnection := GENZConnection
        else
        begin FZInternalDatabase:=TUniConnection.Create(nil);
       ...
       try
        ...
        FZConnection.Username:=alist.Values['User_Name'];
        ...
        FZConnection.Server:=alist.Values['HostName'];
        FZConnection.Database:=alist.Values['Database'];

        if alist.Values['Database Protocol'] ='firebird-1.5' then
          FZConnection.ProviderName := 'InterBase'
          else
        if alist.Values['Database Protocol'] ='mysql' then
          FZConnection.ProviderName := 'MySQL'
          else
        if alist.Values['Database Protocol'] ='mysql-4.1' then
          FZConnection.ProviderName := 'MySQL'
          else

        if alist.Values['Database Protocol'] ='mssql' then
          FZConnection.ProviderName := 'SQL Server'
          else

        if alist.Values['Database Protocol'] ='oracle' then
          FZConnection.ProviderName := 'Oracle'
          else
          FZConnection.ProviderName := alist.Values['Database Protocol']   ;




        FZConnection.SpecificOptions.Add(alist.Values['Property1']);
        FZConnection.SpecificOptions.Add(alist.Values['Property2']);
        FZConnection.SpecificOptions.Add(alist.Values['Property3']);
        FZConnection.SpecificOptions.Add(alist.Values['Property4']);
        FZConnection.SpecificOptions.Add(alist.Values['Property5']);
        FZConnection.SpecificOptions.Add(alist.Values['Property6']);
        FZConnection.SpecificOptions.Add(alist.Values['Property7']);
        FZConnection.SpecificOptions.Add(alist.Values['Property8']);
        FZConnection.SpecificOptions.Add(alist.Values['Property9']);
  ...
  {$IFDEF USEZEOS}
       ...
       begin
        ...
        begin FZInternalDatabase:=TZConnection.Create(nil);
        FZConnection:=FZInternalDatabase;
        end;
       end;
       if FZConnection.Connected then
...
{$ENDIF}
  ...
  begin
   ...
   if not assigned(baseinfo.FZConnection) or (not baseinfo.FZConnection.Connected) then
...
{$ENDIF}
     ...
     rpdatazeos:
      ...
      {$IFDEF USEUNIDAC}
       ...
       FSQLInternalQuery:=TUniQuery.Create(nil);
{$ENDIF}
...
{$IFDEF USERPDATASET}
 ...
 {$ENDIF}
 {$IFNDEF FPC}
       FSQLInternalQuery:=TJvMemoryData.Create(nil);
 {$ENDIF}
{$ENDIF}
{$IFNDEF USERPDATASET}
...
{$ENDIF}
     ...
     rpdatazeos:
      begin
{$IFDEF USEUNIDAC}
  if Not (FSQLInternalQuery is TUniQuery) then
       begin
        FSQLInternalQuery.Free;
        FSQLInternalQuery:=nil;
        FSQLInternalQuery:=TUniQuery.Create(nil);
...
{$ENDIF}
    ...
    rpdatazeos:
     begin
{$IFDEF USEUNIDAC}
      TUniQuery(FSQLInternalQuery).Connection:=
       baseinfo.FZConnection;
      TUniQuery(FSQLInternalQuery).SQL.Text:=SQLsentence;
{$ENDIF}
...
{$ENDIF}
      ...
      try
{$IFNDEF FPC}
       // TJvMemoryData(FSQLInternalQuery).IndexName:='';
       // TJvMemoryData(FSQLInternalQuery).IndexFieldNames:='';
{$ENDIF}
...
{$IFNDEF FPC}
   {$IFDEF USERPDATASET}
         TJvMemoryData(FSQLInternalQuery).FieldDefList.Clear;
   {$ENDIF}
         // TJvMemoryData(FSQLInternalQuery).FieldDefs.Clear;

{$IFDEF FPC}
         FillClientDatasetFromFile(TJvMemoryData(FSQLInternalQuery),baseinfo.FMyBasePath+FMyBaseFields,afilename,FMyBaseIndexFields);
{$ENDIF}
        end
        else
        begin
{$IFNDEF FPC}
        // TJvMemoryData(FSQLInternalQuery).IndexFieldNames:=FMyBaseIndexFields;
         // TJvMemoryData(FSQLInternalQuery).LoadFromFile(afilename);
{$ENDIF}
{$IFDEF FPC}
         // TJvMemoryData(FSQLInternalQuery).LoadFromFile(afilename);
...
{$IFNDEF FPC}
       ...
       begin
      //  TJvMemoryData(FSQLInternalQuery).IndexDefs.Clear;
      //  TJvMemoryData(FSQLInternalQuery).FieldDefs.Clear;
        // TJvMemoryData(FSQLInternalQuery).IndexDefs.Add('IPRIM',FMyBaseIndexFields,[]);
       //  TJvMemoryData(FSQLInternalQuery).IndexFieldNames:=FMyBaseIndexFields;
       end;
{$ENDIF}
       ...
       try
        ...
        begin
         ...
         begin
             {$IFDEF USERPDATASET}
{$IFNDEF FPC}
         CombineAddDataset(TJvMemoryData(FSQLInternalQuery),TRpDatainfolist(Collection).Items[index].Dataset,FGroupUnion);
{$ENDIF}
{$IFDEF FPC}
...
{$ENDIF}
       ...
       begin
        ...
        begin
         try
          {$IFDEF USERPDATASET}
          if (TMemDataset(FSQLInternalQuery).Filter<>afilter) then
          begin
           TMemDataset(FSQLInternalQuery).Filtered:=false;
           TMemDataset(FSQLInternalQuery).Filter:=afilter;
           TMemDataset(FSQLInternalQuery).Filtered:=true;
          end;
           {$ENDIF}
         except
          on E:Exception do
        ...
        begin
          {$IFDEF USERPDATASET}
         if TMemDataset(FSQLInternalQuery).Filtered then
         begin
          TMemDataset(FSQLInternalQuery).Filtered:=false;
          TMemDataset(FSQLInternalQuery).Filter:=afilter;
         end;
...
{$IFDEF USEUNIDAC}
       ...
       if datainfosource.cached then
        ...
        FMasterSource.DataSet:=datainfosource.Dataset;
       AssignParamValuesZ(TUniQuery(FSQLInternalQuery),datainfosource.Dataset);
{$ENDIF}
...
{$ENDIF}
     ...
     rpdatamybase:
      begin
{$IFNDEF FPC}
     //  TJvMemoryData(FSQLInternalQuery).MasterFields:=MyBaseMasterFields;
     //  TJvMemoryData(FSQLInternalQuery).MasterSource:=FMasterSource;
      {$IFDEF USERPDATASET}
       if datainfosource.cached then
        FMasterSource.DataSet:=datainfosource.CachedDataset
       else  {$ENDIF}
...
{$ENDIF}
      ...
      rpdatazeos:
       begin
{$IFDEF USEUNIDAC}
        TUniQuery(FSQLInternalQuery).ParamByName(param.Name).DataType:=atype;
        TUniQuery(FSQLInternalQuery).ParamByName(param.Name).Value:=avalue;
{$ENDIF}
{$IFDEF USEZEOS}
...
{$ENDIF}
  ...
  aQuery.Connection := FZConnection ;
  aQuery.UniDirectional := False ;
  aQuery.FetchRows := 1 ;
  aQuery.SQL.Text := 'select * from ' + atable ;
  aQuery.Open ;
   fieldlist.Clear;
...
{$ENDIF}
  ...
  rpdatazeos:
   begin
    ...
    FSQLInternalQuery:=TUniQuery.Create(nil);
    TUniQuery(FSQLInternalQuery).Connection:=FZConnection;
    TUniQuery(FSQLInternalQuery).SQL.Text:=SQLsentence;
...
{$ENDIF}
   ...
   {$IFDEF USEUNIDAC}
    if assigned(astream) then
      begin
       TUniQuery(FSQLInternalQuery).ParamByName(paramName).DataType:=ftBlob;
       TUniQuery(FSQLInternalQuery).ParamByName(paramName).LoadFromStream(astream,ftBlob);
      end
      else
      begin
       TUniQuery(FSQLInternalQuery).ParamByName(paramName).DataType:=
        VariantTypeToDataType(avariant);
       TUniQuery(FSQLInternalQuery).ParamByName(paramName).Value:=avariant;
      end;
   {$ENDIF}
...
{$ENDIF}
 ...
 {$ENDIF}
   ...
   rpdatazeos:
    begin
  {$IFDEF USEUNIDAC}
     TUniQuery(FSQLInternalQuery).ExecSQL;
 {$ENDIF}
...
begin
 counter:=0;
 aresult:=TMemDataset.Create(nil);
...
{$ENDIF}
  ...
  begin
{$IFNDEF FPC}
   ParseFields(client.FieldDefs[0].Name ,groupfields);
...
var
 ...
 DReportgroups,DReportgroups2:TVirtualTable;
 Dreports:TVirtualTable;
 groupcode:Integer;
 grouppath:string;
 sqltext:string;
 havegroup:Boolean;
...
begin
 ...
 try
  ...
  try
   ...
   DReportGroups:=TVirtualTable.Create(nil);
   DReportGroups2:=TVirtualTable.Create(nil);
   DReports:=TVirtualTable.Create(nil);
   try
...
begin
 ...
 if dtype=rpdatazeos then
  if Not EqualParamValuesZ(TUniQuery(datainfoitem.dataset),DataSource.Dataset) then
   reopen:=true;
{$ENDIF}
...
{$IFDEF USEUNIDAC}
  if dtype=rpdatazeos then
   AssignParamValuesZ(TUniQuery(datainfoitem.dataset),DataSource.Dataset);
{$ENDIF}
