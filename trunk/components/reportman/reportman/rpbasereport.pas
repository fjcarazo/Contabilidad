...
type
 ...
 TCustomFunction=class(TIdenFunction)
  private
  protected
   FReport:TRpBaseReport;
   function GetRpValue:TRpValue;override;
  public
   ...
   property    Report:TRpBaseReport read FReport;
 ...
 TIdenSetRepeatCount=class(TIdenFunction)
  private
   FReport:TRpBaseReport;
  protected
   function GetRpValue:TRpValue;override;
  ...
  TIdenDataAsString=class(TIdenFunction)
  private
   FReport:TRpBaseReport;
  protected
   varname:string;
  ...
  TIdenLocate=class(TIdenFunction)
  private
   FReport:TRpBaseReport;
  protected
   function GetRpValue:TRpValue;override;
 ...
 TIdenStripHtml=class(TIdenFunction)
  private
  protected
   function GetRpValue:TRpValue;override;
  public
 ...
 TRpBaseReport=class(TComponent)
  private
   ...
   procedure StopWork;
    procedure SetIdenSetRepeatCount(const Value: TIdenSetRepeatCount);
  protected
    ...
    FidenEof:TIdenEof;
    FIdenSetRepeatCount: TIdenSetRepeatCount;
    FCustomFunction: TCustomFunction;
    FIdenLocate: TIdenLocate;
    FIdenStripHtml: TIdenStripHtml;
    ...
    FidencurrentSection:TIdenReportVar;
    FIdenfirstsection:TIdenReportVar;
    FIdenDataAsString : TIdenDataAsString ;
    // Other
...
{$ENDIF}
  ...
  public
   ...
   property IdenSetRepeatCount : TIdenSetRepeatCount  read FIdenSetRepeatCount write FIdenSetRepeatCount;
   property CustomFunction : TCustomFunction read fCustomFunction;
   property IdenLocate : TIdenLocate read FIdenLocate;
   property IdenDataAsString : TIdenDataAsString read fIdenDataAsString;


   property IdenStripHtml : TIdenStripHtml read FIdenStripHtml;
...
begin
 ...
 FIdenSetRepeatCount := TIdenSetRepeatCount.Create(nil);
 FIdenSetRepeatCount.varname:='SETREPEATCOUNT';
 FIdenSetRepeatCount.FReport:=self;

 FCustomFunction :=  TCustomFunction.create(nil);
 FCustomFunction.varname:='CUSTOM';
 FCustomFunction.FReport:=self;

 FIdenLocate := TIdenLocate.Create(nil);
 FIdenLocate.varname:='AlIASLOCATE';
 FIdenLocate.FReport:=self;

 FIdenStripHtml:= TIdenStripHtml.Create(nil);
 FIdenStripHtml.varname:='STRIMHTML';


 FIdenDataAsString := TIdenDataAsString.Create(nil);
 FIdenDataAsString.varname:='DATASETASSTRING';
 FIdenDataAsString.FReport:=self;
...
{$ENDIF}
 ...
 FIdenSetRepeatCount.free;

 FCustomFunction.free;

 FIdenLocate .free;

 FIdenStripHtml.free;
 FIdenDataAsString.free;
...
constructor TIdenLocate.Create(AOwner: TComponent);
begin
 inherited Create(AOwner);
 FParamcount:=3;
 IdenName:='AliasLocate';
// Help:=SRpInt;
 model:='function '+'AliasLocate'+'(alias,field:string;value : variant):Boolean';
 help :='alias ; the alias for the dataset. field ; the filed to use in the locate. Value a value to use in the locate';
// aParams:=SRpEof;
end;

function TIdenLocate.GetRpValue: TRpValue;
var
 aliasname :string;
...
begin
 ...
 if (not ( (VarType(Params[1])=varString) or (VarType(Params[1])=varOleStr) )) then
   Raise TRpNamedException.Create(SRpEvalType,
         IdenName);
 result := false ;
 aliasname:=Vartostr(Params[0]);
 index:=FReport.DataInfo.IndexOf(aliasname);
 if index<0 then
  begin
   Result:='aliasname ' + aliasname +' not found!' ;
    exit;
  end;
{$IFDEF USERPDATASET}
 if FReport.DataInfo.Items[index].Cached then
...
{$ENDIF}
 ...
 if Not dataset.Active then
  begin
  exit;
  Result:='Data not open!' +  FReport.DataInfo.Items[index].Alias ;
  end;
  Result:=dataset.Locate(vartostr(Params[1]),vartostr(Params[2]),[]);
end;
{ TIdenStripHtml }

{ TIdenStripHtml }

constructor TIdenStripHtml.Create(AOwner: TComponent);
begin
  inherited;
 FParamcount:=1;
 IdenName:='StripHtml';
// Help:=SRpInt;
 model:='function '+'StripHtml'+'(HTML:string):String';
 help :='Strips all HTML tags from a string';
// aParams:=SRpEof;
end;

function TIdenStripHtml.GetRpValue: TRpValue;
var
 ATemString : String ;
begin
  ATemString := '' ;
   if VarType(Params[0])=varString then
     begin
      ATemString := Params[0] ;
      while pos('<',ATemString) > 0 do
        begin
          if pos('>',ATemString) > 0 then
            begin
               if pos('>',ATemString) <  pos('<',ATemString) then
                 break ;
               Delete(ATemString, pos('<',ATemString),(pos('>',ATemString)- pos('<',ATemString))+1);
...
constructor TCustomFunction.Create(AOwner: TComponent);
begin
 inherited Create(AOwner);
 fParamCount := 6 ;
 IdenName:='CUSTOM';
 model:='function '+'CUSTOM'+'(functionname,value1,value2,value3,vlue4,value5): variant';
// aParams:=SRpEof;
 help :='custom ; the custom fuctions for programmers using reportman. Please refere to the manual of the program used for details on this function.';
end;
...
constructor TIdenSetRepeatCount.Create(AOwner: TComponent);
begin
  inherited;
 FParamcount:=2;
 IdenName:='SetRepeatCount';
// Help:=SRpInt;
 model:='procedure '+' repeatcount'+'(ALIAS:string;COUNT:Number)';
 help :='Set the repeat count for a Alias so you can make multyple barcodes based on a qty field';
// aParams:=SRpEof;

end;

function TIdenSetRepeatCount.GetRpValue: TRpValue;
var
 aliasname:string;
 index:integer;
 dataset:TDataset;
begin
 ...
 if (not ( (VarType(Params[0])=varString) or (VarType(Params[0])=varOleStr) )) then
   Raise TRpNamedException.Create(SRpEvalType,
         IdenName);
 if (not ( (VarType(Params[1])=varInteger) or (VarType(Params[1])=varnull)  or (VarType(Params[1])=varDouble)) ) then
   Raise TRpNamedException.Create(SRpEvalType,
         IdenName);
 Result:=true;
 aliasname:=String(Params[0]);
...
{$ENDIF}
 ...
 if Not dataset.Active then
  exit;
  {$IFDEF USERPDATASET}
 if dataset is  TRpDataset then
  if VarType(Params[1])<>varnull then
  (dataset as TRpDataset).RepeatRecords :=  Params[1] else
  (dataset as TRpDataset).RepeatRecords := 0 ;
  {$ENDIF}


end;

procedure TRpBaseReport.SetIdenSetRepeatCount(
  const Value: TIdenSetRepeatCount);
begin
  FIdenSetRepeatCount := Value;
end;

{ TIdenDataAsString }

constructor TIdenDataAsString.Create(AOwner: TComponent);
begin
 inherited ;
 FParamcount:=1;
 IdenName:='DataSetAsString';
 model:='function '+'DataSetAsString'+'(alias :string; ):String';
 help :='Dataset as string can make a long string of your datase alias is the name of the dataset';
// aParams:=SRpEof;
...
begin
 ...
 eval.AddIden('SETREPEATCOUNT',FIdenSetRepeatCount);
 eval.AddIden('CUSTOM',fCustomFunction);
 eval.AddIden('ALIASLOCATE',FIdenLocate);
 eval.AddIden('DATASETASSTRING',FIdenDataAsString);
 eval.AddIden('STRIPHTML',FIdenStripHtml);
...
function TCustomFunction.GetRpValue: TRpValue;
...
begin
 Result := '';
 if assigned(CustomRoutine) then
   CustomRoutine(self,result) ;
end;


function TIdenDataAsString.GetRpValue: TRpValue;
var
 aliasname :string;
 ...
 if Not dataset.Active then
  ...
  while not dataset.eof do
    begin
     if Result <> '' then
       Result:= Result +  ',' ;
     Result:= Result +  dataset.fields[0].asstring ;
     dataset.next ;
    end;
