...
type
  ...
  public
    AUndoList : TStringlist ;
    ARedoList : TStringlist ;
    { Public declarations }
...
begin
 ...
 AUndoList := TStringlist.create ;
 ARedoList := TStringlist.create ;
...
begin
 ...
 begin
  if (TFRpObjInspVCL(Owner).OpenDialog1.FilterIndex<=4) then
  begin
...
procedure TFRpObjInspVCL.UndoAction;
...
var
 ...
 index:integer;
 alist:TStringList;
 Helper1,helper2,helper3 : String ;
 NewHeigth : Double ;
begin
 ClearMultiSelect;
 if AUndoList.Count = 0 then exit ;
 Helper1  := '' ;
 if AUndoList.Count > 0 then
    Helper1 := AUndoList[AUndoList.Count-1] ;
 AUndoList.Delete(AUndoList.Count-1);
 Helper3 := '' ;
 if Helper1 = '' then exit ;

 Helper2 := copy(Helper1,1,pos('=',Helper1)-1);
 delete(Helper1,1,pos('=',Helper1));
 if pos(';',Helper1) > 0 then
    begin
     Helper3 := copy(Helper1,pos(';',Helper1)+1,length(Helper1)+1);
     Delete(Helper1,pos(';',Helper1),length(Helper1));
    end;
  if pos('WIDTH',Helper1) = 1 then
    begin
      NewHeigth :=  StrToFloatDef(Helper3,0);
    end;
 if pos('HEIGHT',Helper1) = 1 then
    begin
      NewHeigth :=  StrToFloatDef(Helper3,0);
    end;
 if pos('LEFT',Helper1) = 1 then
    begin
      NewHeigth :=  StrToFloatDef(Helper3,0);
    end;
 if pos('TOP',Helper1) = 1 then
    begin
      NewHeigth :=  StrToFloatDef(Helper3,0);
    end;
 ...
 begin
  ...
  begin
   ...
   if index<0 then
    Raise Exception.Create(SRpClassNotRegistered+':'+compo.classname);
   alist:=TStringList(FClassAncestors.Objects[index]);
   if compo.printitem.Name = helper2 then
     begin

      FSelectedItems.AddObject(compo.ClassName,compo);
      if pos('WIDTH',Helper1) = 1 then
      begin
        TRpCommonPosComponent(compo.printitem).Width := trunc( NewHeigth) ;
      end else
      if pos('HEIGHT',Helper1) = 1 then
      begin
        TRpCommonPosComponent(compo.printitem).Height := trunc( NewHeigth) ;
      end else
      if pos('LEFT',Helper1) = 1 then
      begin
        TRpCommonPosComponent(compo.printitem).PosX := trunc( NewHeigth) ;
      end else
      if pos('TOP',Helper1) = 1 then
      begin
        TRpCommonPosComponent(compo.printitem).PosY := trunc( NewHeigth) ;
      end;
      compo.UpdatePos ;
     end;
  end;
