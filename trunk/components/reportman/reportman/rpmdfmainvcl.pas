...
begin
 ...
 fobjinsp.AUndoList.OnChange := UndolistChange ;
 Undo1.Enabled := false ;
...
procedure TFRpMainFVCL.Undo1Click(Sender: TObject);
begin
  if Assigned(fobjinsp) then
   begin
     fobjinsp.UndoAction ;
     Undo1.Caption := '' ;
     if  fobjinsp.AUndoList.Count > 0 then
     Undo1.Caption := 'Undo '+fobjinsp.AUndoList[fobjinsp.AUndoList.Count-1] +' (' + IntToStr(fobjinsp.AUndoList.Count) +')' ;

   end;
end;

procedure TFRpMainFVCL.UndolistChange(Sender: TObject);
begin
  if Assigned(fobjinsp) then
   begin
     Undo1.Caption := 'Undo' ;
