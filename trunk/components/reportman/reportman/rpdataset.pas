...
type
 ...
 TRpDataset=class(TVirtualTable)


  private

   FCopyDataset:TVirtualTable;

   FDataset:TDataset;
   procedure SetDataset(Value:TDataset);
    procedure SetRepeatRecords(const Value: Integer);
  private
    FRepeatCount: integer;
    FRepeatRecords: Integer;
  protected
  ...
  public
   ...
   procedure DoClose;
   procedure DoNext;
   procedure DoNextRec(ADataSet: TDataSet);
   procedure DoPrior;
   constructor Create(AOwner:TComponent);override;
  published
   property Dataset:TDataset read FDataset write SetDataset;

   property RepeatRecords : Integer  read FRepeatRecords write SetRepeatRecords;
 end;
...
begin
 inherited Create(AOwner);
  FCopyDataset:=TVirtualTable.Create(Self);
...
begin
 if assigned(FCopyDataset) then
 FCopyDataset.Close;
 Close;
end;
procedure TRpDataset.DoNextRec(ADataSet: TDataSet);
begin
 if RepeatRecords > 1 then
   begin
     if FRepeatCount < (RepeatRecords -1) then
        inc(FRepeatCount) else
        begin
          aDataset.Next ;
          FRepeatCount := 0 ;
        end;
...
procedure TRpDataset.SetRepeatRecords(const Value: Integer);
begin
  FRepeatRecords := Value;
end;
