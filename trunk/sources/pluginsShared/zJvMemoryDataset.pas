...
function TJvMemoryData.DataSetLocateThrough(DataSet: TDataSet; const KeyFields: string;  const KeyValues: Variant; Options: TLocateOptions): Boolean;
var
  FieldCount: Integer;
  Fields: TList;
...
{procedure TJvMemoryData.AssignRecord(Source, Dest: TDataSet; ByName: Boolean);
var
  I: Integer;
  F, FSrc: TField;
begin
...
begin
  ...
  begin
   ...
   begin
     // done : Pieter added simple filter
   {  if (Filter <> '') and ((FRecordPos >= 0) and (FRecordPos < RecordCount)) then
        begin
           if pos('=',Filter) <> 0 then
            begin
               afield := copy(Filter,1,pos('=',Filter)-1);
               AValue := copy(Filter,pos('=',Filter)+1,Length(Filter));
               if (AValue <> '') and (FindField(afield) <> nil) then
                begin
                 SaveState := SetTempState(dsFilter);
                   try
                    RecordToBuffer(Records[FRecordPos], TempBuffer);
                    Result :=  FindField(afield).AsString = AValue ;
                    except
                      Application.HandleException(Self);
                    end;
                    RestoreState(SaveState);
...
begin
  ...
  // Done : Pieter added support for change knowledge
  if FApplyMode <> amNone then
     begin
       AddStatusField;
       HideStatusField;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (F <> nil) and (F.DataType <> ftAutoInc) then
       try
        if (F.DataType in [ftString,ftBlob,ftMemo,ftWideString]) then
           begin
             if VarToStr(Source.Fields[I].Value) = '' then
                 F.Value := Source.Fields[I].Value
                 else
                 begin
                 TemStr := UTF8Decode(Source.Fields[I].AsString) ;
                 if (TemStr <> ''){ and (Length(TemStr) >= length(Source.Fields[I].AsString))} then
                 F.AsString := TemStr
                  else
         ...
         F.AsInteger := Source.Fields[I].AsInteger;
       end;
    end;
  end
  else
...
procedure TJvMemoryData.AssignRecord(Source : TDataSet; Dest: TDataSet;
  ByName: Boolean);
begin

end;

procedure TJvMemoryData.CreateFieldDefs(AFieldsNode: TJanXmlNode2;
  aFieldDefs: TFieldDefs);
begin

end;

function TJvMemoryData.DataSetToXmlString(
  aToDataset: TJvMemoryData): String;
begin

end;

procedure TJvMemoryData.ExportData(ARowsNode: TJanXmlNode2;
  aDataSet: TDataSet);
begin

end;

procedure TJvMemoryData.ExportFields(AFieldsNode: TJanXmlNode2;
  aFieldDefs: TFieldDefs);
begin

end;

procedure TJvMemoryData.InsertData(ARowNode: TJanXmlNode2;
  aDataSet: TDataSet);
begin

end;

procedure TJvMemoryData.LoadXMLFromString(AXml: String;
  aToDataset: TJvMemoryData);
begin
