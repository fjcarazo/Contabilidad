...
type
  ...
  private
    ...
    Function  GetGrpIDFilter(AliasName :string ):String; 
...
Function TfmGLLedgerOptions.GetGrpIDFilter(AliasName :string ):String; 
Var
  ID,TID:integer;
  Function Grp:String;
  begin
...
if ID>0 then
//kalmen , added WReportingGroup with alias Account
if AliasName <> '' then
Result := ' AND ( '+AliasName+'.WReportingGroup'+Grp+'ID='''+IntToStr(ID)+''')'
else
...
begin
  ...
  add(dmDatabase.SQLList.GetFormatedSQLByName('GLLedgerOptions_Transactions') + GetGrpIDFilter('Account') ) ;
