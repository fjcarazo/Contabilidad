...
begin
  ...
  dmDatabase.qrySalesByGroup.ParamByName('WGroupTypeID').AsInteger := GroupID;



  dmDatabase.qrySalesByGroup.Open;
  dmDatabase.qrySalesByGroup.Filtered := False;
  dmDatabase.qrySalesByGroup.Filter := 'SDescription >= ''' + dmDatabase.tblReportingOptions.FieldByName('SSalesByGroupFromGroup').Asstring + ''' AND SDescription <= ''' + dmDatabase.tblReportingOptions.FieldByName('SSalesByGroupToGroup').Asstring + '''  ' ;
