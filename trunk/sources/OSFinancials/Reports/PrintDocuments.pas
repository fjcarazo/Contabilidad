...
uses Database, Main, GlobalFunctions, LanguageChoice, 
   PosRoutines, TcVariables, DosTextReports, DatabaseAccess,
  XRoutines, FreeReportsUnit, OSFGeneralRoutines, UDMTCCoreLink;
...
begin
 ...
 BtnCancel.Caption:= GetTextLang(168);
  ...
  CbDocTypeNew.ItemIndex := CbDocTypeNew.Items.IndexOfObject(Tobject(idDocType));
 ...
 // dmDatabase.tblDocLine.Open;
  dmDatabase.tblGroups.Open;
   ...
   begin
      ...
      dmDatabase.tblDocHeader.sql.Text :='select * from dochead where SDocNo=' + QuotedStr(dmDatabase.qryDocHeadSDocNo.AsString) + ' and WTypeID ='  + IntToStr(idDocType) ;
   ...
   //   dmDatabase.tblDocHeader.Locate('SDocNo;WTypeID',VarArrayOf([dmDatabase.qryDocHeadSDocNo.AsString,idDocType]) ,[]);
      ...
      dmDatabase.tblDocLine.sql.text := 'select  * from docline where WDocID =' + IntToStr(dmDatabase.qryDocHeadWDocID.AsInteger) ;
      dmDatabase.tblDocLine.open;
...
begin
  ...
  if Not (dmDatabase.qryDocHead.FieldByName('BPrinted').Asinteger > 0) then
    ABrush.Color:=clInfoBk
  else if Not (dmDatabase.qryDocHead.FieldByName('BPosted').Asinteger > 0) And not Highlight  then
   AFont.Color:=clRed;
