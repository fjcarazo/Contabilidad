...
uses
  ...
  StdCtrls, ExtCtrls, DBCtrls, Buttons, ComCtrls,DB;
...
uses Main, Database, Lookup, InvoicedDetail, HTMLHelpAPI, LanguageChoice,
  ...
  InvoiceTaxDetail, UDMTCCoreLink;
...
begin
 ...
 DMTCCoreLink.LoadImage(Bitprn1.Glyph,'Printer');
   ...
   Caption := GetTextLang(390);//Header 390
   Label1.Caption := GetTextLang(1079); //Type
   Label3.Caption := GetTextLang(242);//Which Year 242
   Label4.Caption := GetTextLang(286);//From Date 286
   Label5.Caption := GetTextLang(208);//To 208
   Label2.Caption := GetTextLang(391);//Tax Code 391
   dbcbShowDetails.Caption := GetTextLang(392);//Show Detail 392
...
begin
  ...
  begin
    ...
    try
    qrInvoicedTaxDetail.QGenTax.Connection :=   dmDatabase.ZMainconnection ;
    dmDatabase.tblSysVars.Open;
