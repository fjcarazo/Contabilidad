...
DMTCCoreLink.LoadImage(Bitprn1.Glyph,'Printer');
  ...
  cdRepGoups.Items[1] := TDataBaseRoutines.GetTypeDescription(22) ;
  cdRepGoups.Items[2] := TDataBaseRoutines.GetTypeDescription(23) ;
  ...
  if dmDatabase.SetOfBooksPropertys.DataParameter.CBUseCurrentDateForReports then
     begin
       ...
       if TDataBaseRoutines.GetPeriodEndDate(26) < date then
        wwDBDateTimePickerAgeDate.Field.AsDateTime := TDataBaseRoutines.GetPeriodEndDate(26)
        else   wwDBDateTimePickerAgeDate.Field.AsDateTime := date ;
...
begin
 ...
 // Sylvain: There is a problem when change text in runtime
  ...
  with dmDatabase do begin
   ...
   FillTmpAgeTable(True,tblReportingOptions.FieldByName('DDebtorAgeDateLimit4').AsDateTime,tblReportingOptions.FieldByName('DDebtorAgeDateLimit3').AsDateTime,tblReportingOptions.FieldByName('DDebtorAgeDateLimit2').AsDateTime, tblReportingOptions.FieldByName('DDebtorAgeCurrentDate').AsDateTime,Not(dbcbOpenItem.Checked=False),dbcbShowTransactions.Checked);
    _SetProgress(-2);
