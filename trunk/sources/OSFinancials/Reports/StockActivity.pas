...
begin
 ...
 // dmDatabase.tblPeriods.Locate('WPeriodID',PeriodID+13, []);
  aDMCoreDocData.ZQStockTrans.SQL.Text := 'select stocktrn.*,dochead.SDocno from stocktrn left join dochead on stocktrn.WDOCID = dochead.WDOCID where  WStockID =  ' +  dmDatabase.tblStockWStockID.AsString + ' AND cast(stocktrn.DDate as date) >= ' + TDatabaseTableRoutines.GetFirebirdsSqlForDate(aLedgerPeriodeItem.StartDate) + ' AND cast(stocktrn.DDate as date) <= ' + TDatabaseTableRoutines.GetFirebirdsSqlForDate(aLedgerPeriodeItem.EndDate)
   + ' And   (stocktrn.WDoctypeID=10 Or stocktrn.WDocTypeID=11 or stocktrn.WDocTypeID=13) ' ;
  ...
  aDMCoreDocData.ZQStockTrans.SQL.Text := 'select * from stocktrn where WStockID =   ' + dmDatabase.tblStockWStockID.AsString +  ' AND cast(stocktrn.DDate as date) >= ' + TDatabaseTableRoutines.GetFirebirdsSqlForDate(aLedgerPeriodeItem.StartDate) + ' AND cast(stocktrn.DDate as date) <= ' + TDatabaseTableRoutines.GetFirebirdsSqlForDate(aLedgerPeriodeItem.EndDate)
    + ' And   (WDoctypeID=10 Or WDocTypeID=11 or WDocTypeID=13) ' ;
