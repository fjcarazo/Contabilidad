...
begin
  if Abs(dmDatabase.tblTmpAgeAnalFTOTAL.AsFloat)<0.001 then PrintBand:=False;
  ...
  qrlTotal.Caption := FloatToStrT( dmDatabase.tblTmpAgeAnalFTOTAL.AsFloat, ffFixed, 18, 2);
end;
...
//    qrl60Amnt.Caption := '';
  ...
  dmDatabase.ZQOILinks.params[0].AsInteger := dmDatabase.qryCreditAgeWTransactionID.AsInteger ;
...
begin
  ...
  if ((Abs(dmDatabase.tblTmpAgeAnalFTOTAL.AsFloat)< Abs(dmDatabase.tblReportingOptions.FieldByName('FCREDITorAgeMinimumAmount').AsFloat)) AND (dmDatabase.tblReportingOptions.FieldByName('FCREDITorAgeMinimumAmount').AsFloat<>0))
     Or
     ((dmDatabase.tblReportingOptions.FieldByName('BCreditorAgeIgnoreZeroBal').Asinteger = 1) AND (FixAmount(dmDatabase.tblTmpAgeAnalFTOTAL.AsFloat)=0))
     then begin
   PrintBand:=False;
   exit;
  end;
  TotalTotal := TotalTotal  + dmDatabase.tblTmpAgeAnalFTOTAL.AsFloat ;
  //  + CurrentTotal + _30daysTotal + _60daysTotal + _90daysTotal;
  ...
  begin
   ...
   qrlTotalg.Caption := FloatToStrT(dmDatabase.tblTmpAgeAnalFTOTAL.AsFloat , ffFixed, 18, 2);
  end;
  if (dmDatabase.tblReportingOptions.FieldByName('BCreditorAgeAccountDetail').Asinteger = 1) then
  begin
