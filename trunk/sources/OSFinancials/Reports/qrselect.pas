...
begin
  ...
  SaveIndex := SelQRPrinter.PrinterIndex;
  SelQRPrinter.PrinterIndex := cbPrinters.ItemIndex;
    // Quick and easy way to see if the printer supports the printing
    // of multiple copies
  SaveCopies := SelQRPrinter.Copies;
  SelQRPrinter.Copies := 2;
  SelQRPrinter.Copies := SaveCopies;
  SelQRPrinter.PrinterIndex := SaveIndex;
end;
...
var
  Dummy : array [0..100] of ansichar ;
begin
// done : Pieter windows xp and 2000 this works not sure if it does on win98
StrCopy(PAnsiChar(dummy[0]),PAnsichar('RUNDLL32 PRINTUI.DLL,PrintUIEntry /p /n "'+cbPrinters.text +'"' + #0))  ;
  WinExec(PAnsiChar(Dummy[0]), 1);
end;
