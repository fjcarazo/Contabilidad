...
begin
  ...
  begin
    ...
    LYTotal := 0;
    BudgetTotal := 0;
    Total := GetRangeTotal(0,tblReportingOptions.FieldByName('WIncomeStmntFromDateID').AsInteger   ,tblReportingOptions.FieldByName('WIncomeStmntToDateID').AsInteger   ,dmDatabase.qrBalSheetAccountWAccountID.AsInteger,IncludeOpeningBals,(qrBalSheetAccountBIncomeExpense.Asinteger = 1));
    LYTotal := GetRangeTotal(-1,tblReportingOptions.FieldByName('WIncomeStmntFromDateID').AsInteger-13,tblReportingOptions.FieldByName('WIncomeStmntToDateID').AsInteger-13,dmDatabase.qrBalSheetAccountWAccountID.AsInteger,IncludeOpeningBals,(qrBalSheetAccountBIncomeExpense.Asinteger = 1));
    BudgetTotal := GetRangeTotal(1,tblReportingOptions.FieldByName('WIncomeStmntFromDateID').AsInteger,tblReportingOptions.FieldByName('WIncomeStmntToDateID').AsInteger,dmDatabase.qrBalSheetAccountWAccountID.AsInteger,IncludeOpeningBals,(qrBalSheetAccountBIncomeExpense.Asinteger = 1));
    ...
    begin
      ...
      Debit1.Caption := '';
      Debit2.Caption := '';
      if (BudgetTotal <> 0 ) and ((qrBalSheetAccountBSubAccounts.Asinteger <> 1) and fmIncomeStatementOptions.dbcboxShowSubAccounts.Checked) then
        begin
        if fmIncomeStatementOptions.cbBudget.Checked then
          begin
             Debit2.Caption := FloatToStrTd2c(BudgetTotal , ffFixed, 18, sDigits,true);
             GroupBgTotal := GroupBgTotal + BudgetTotal;
          end ;
        end;
      if (LyTotal <> 0) and ((qrBalSheetAccountBSubAccounts.Asinteger <> 1) and fmIncomeStatementOptions.dbcboxShowSubAccounts.Checked) then
        begin
         if fmIncomeStatementOptions.cbLastYear.Checked then
            begin
              Debit2.Caption := FloatToStrTd2c(LyTotal , ffFixed, 18, sDigits,true);
    ...
    begin
      ...
      Debit1.Caption := FloatToStrTd2c(Total , ffFixed, 18, sDigits,true);
      if fmIncomeStatementOptions.cbBudget.Checked then
        Debit2.Caption := FloatToStrTd2c(BudgetTotal , ffFixed, 18, sDigits,true)
      else
        Debit2.Caption := FloatToStrTd2c(LyTotal , ffFixed, 18, sDigits,true);
    ...
    qrVariance.Caption := '' ;
    if not (fmIncomeStatementOptions.dbcboxShowSubAccounts.Checked and (qrBalSheetAccountBSubAccounts.Asinteger = 1)) then
    begin
    if fmIncomeStatementOptions.cbBudget.Checked then

      qrVariance.Caption := FloatToStrTd2c(Total - BudgetTotal , ffFixed, 18, sDigits,true)
    else
      qrVariance.Caption := FloatToStrTd2c(Total - LyTotal , ffFixed, 18, sDigits,true);
    end;
    //PL
...
begin
  // This/Ly Year L/P
    ...
    NetProfDt.Caption := FloatToStrTd2c(GrandPLTyTotal , ffFixed, 18, sDigits,true) ;
    if fmIncomeStatementOptions.cbBudget.Checked then
    begin
      NetProfCr.Caption := FloatToStrTd2c(GrandBgTotal , ffFixed, 18, sDigits,true) ;
      qrVarLp.Caption := FloatToStrTd2c(GrandPLTyTotal-GrandBgTotal , ffFixed, 18, sDigits) ;
    end
    else
    begin
      NetProfCr.Caption := FloatToStrTd2c(GrandPLLyTotal , ffFixed, 18, sDigits,true)  ;
...
//  qrGroupsTotalLsCr.Caption:='';
  ...
  begin
     ...
     // This year Groups
     qrGroupsTotalThDt.Caption:=FloatToStrTd2c(GroupThTotal, ffFixed, 18, sDigits,true);
     if  fmIncomeStatementOptions.cbBudget.Checked then
     begin
       qrGroupsTotalLYCr.Caption:=FloatToStrTd2c(GroupBgTotal, ffFixed, 18, sDigits,true);
       qrGroupsVar.Caption :=FloatToStrTd2c(GroupThTotal-GroupBgTotal, ffFixed, 18, sDigits,true);
     end
     else
     begin
       qrGroupsTotalLYCr.Caption:=FloatToStrTd2c(GroupLyTotal, ffFixed, 18, sDigits,true);
       qrGroupsVar.Caption :=FloatToStrTd2c(GroupThTotal-GroupLyTotal, ffFixed, 18, sDigits,true);
     end;
  end;
end;
