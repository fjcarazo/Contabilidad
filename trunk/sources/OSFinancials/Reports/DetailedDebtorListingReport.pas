...
Uses
  Database, DatabaseAccess, OSFGeneralRoutines;
...
begin
 ...
 // dmDatabase.tblAccount.Locate('WAccountID', QRDBText5.DataSet.fieldByName('WAccountID').AsInteger, []);
    aLevel := 0 ;
    qrlRep1.Caption :=  TDataBaseRoutines.GetNesteldNameFromGroupID(dmDatabase.tblAccountWReportingGroup1ID.AsInteger,ALevel);
    aLevel := 0 ;
    qrlRep2.Caption :=  TDataBaseRoutines.GetNesteldNameFromGroupID(dmDatabase.tblAccountWReportingGroup1ID.AsInteger,ALevel);
     aLevel := 0 ;
  if UseCreditor<>True then
    qrlSalesman.Caption := TDataBaseRoutines.GetNesteldNameFromGroupID(dmDatabase.tblDebtorWSalesmanID.Asinteger,aLevel)
     else
    qrlSalesman.Caption := '';
