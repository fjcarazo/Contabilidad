...
type
  TqrTrialBalance = class(TQuickRep)
    ...
    ZQThisYearWACCOUNTID: TIntegerField;
    ZQThisYearSACCOUNTCODE: TStringField;
    ZQThisYearSSUBACCOUNTCODE: TStringField;
    ZQThisYearBSUBACCOUNTS: TSmallintField;
    ZQThisYearSDESCRIPTION: TStringField;
    ZQThisYearWACCOUNTTYPEID: TIntegerField;
    ZQThisYearBINCOMEEXPENSE: TSmallintField;
    ZQThisYearWSORTNO: TIntegerField;
    ZQThisYearWSORTNO_1: TIntegerField;
    ZQThisYearWREPORTINGGROUP1ID: TIntegerField;
    ZQThisYearWREPORTINGGROUP2ID: TIntegerField;
    ZQThisYearTOTAL: TFloatField;
    ZQAccounts: TuniQuery;
    ZQAccountsLastyear: TFloatField;
    ZQAccountsBuget: TFloatField;
    ZQAccountsTotal: TFloatField;
    ZQAccountsWACCOUNTID: TIntegerField;
    ZQAccountsSACCOUNTCODE: TStringField;
    ZQAccountsSSUBACCOUNTCODE: TStringField;
    ZQAccountsBSUBACCOUNTS: TSmallintField;
    ZQAccountsSDESCRIPTION: TStringField;
    ZQAccountsWACCOUNTTYPEID: TIntegerField;
    ZQAccountsBINCOMEEXPENSE: TSmallintField;
    ZQAccountsWSORTNO: TIntegerField;
    ZQAccountsWSORTNO_1: TIntegerField;
    ZQAccountsWREPORTINGGROUP1ID: TIntegerField;
    ZQAccountsWREPORTINGGROUP2ID: TIntegerField;
...
var
  ...
  Debit,Credit : Double ;
begin
  if ZQAccountsWACCOUNTID.Value = 0 then begin
   Printband := false;
   exit;
  end;
  with dmDatabase do
  begin
    ...
    //this year
       ...
       Total := Total + ZQAccountsTotal.AsFloat ;
   {  if QThisYearWAccountID.Value =  dmDatabase.SetOfBooksPropertys.CompanyInfo.RetaintEarnings then
       begin
         Total := GetAccountTotal(QThisYearWAccountID.Value,dmDatabase.tblReportingOptions.FieldByName('WTrialBaloDateID').asinteger,5)
       end;  }
     if fmTrialBalanceOptions.CBBeforePosting.Checked then
       begin
        if fmTrialBalanceOptions.CBUpostedonly.Checked then
        Total := 0 ;
        if ZQAccountsWAccountID.Value = tblSysVarsWDebtorsControlID.Value then
           begin
        ...
        if ZQAccountsWAccountID.Value = tblSysVarsWCreditorsControlID.Value then
           begin
            // add periodes and sum
            for x :=  0 to LedgerCalculationClass.Count -1 do
              begin
          ...
          begin
             if LedgerCalculationClass.HasItemOnAccountId(ZQAccountsWAccountID.AsInteger) then
             begin
             LedgerCalculationClass.GetItemOnAccountId(ZQAccountsWAccountID.AsInteger).GetTotalsFromPeriodes(dmDatabase.tblReportingOptions.FieldByName('WTrialBalFromDateID').AsInteger,dmDatabase.tblReportingOptions.FieldByName('WTrialBaloDateID').asinteger,Debit,Credit) ;
             Total := Total + (debit - credit);
             end;
          end
    ...
    if ((ZQAccountsSSUBACCOUNTCODE.Value = '000') and
       (ZQAccountsWAccountID.Value <> tblSysVarsWDebtorsControlID.Value) and (ZQAccountsWAccountID.Value <> tblSysVarsWCreditorsControlID.Value) and
       (ZQAccountsWAccountID.Value <> dmDatabase.SetOfBooksPropertys.CompanyInfo.RetaintEarnings) and (ZQAccountsBSubAccounts.AsInteger = 1)) or (Total = 0) then
    begin
    ...
    BudgetTotal := BudgetTotal +  ZQAccountsBuget.AsFloat;
    if (ZQAccountsSSUBACCOUNTCODE.Value = '000')  and
       (ZQAccountsWAccountID.Value <> dmDatabase.SetOfBooksPropertys.CompanyInfo.DebtorsControlAccount) and (ZQAccountsWAccountID.Value <> dmDatabase.SetOfBooksPropertys.CompanyInfo.CreditorsControlAccount) and
       (ZQAccountsWAccountID.Value <> dmDatabase.SetOfBooksPropertys.CompanyInfo.RetaintEarnings) and (ZQAccountsBSubAccounts.AsInteger = 1) or (BudgetTotal = 0) then
    begin
      //this is a main account
    ...
    LYTotal := LYTotal + ZQAccountsLastyear.AsFloat;
     ...
     if fmTrialBalanceOptions.CBBeforePosting.Checked then
       begin
        ...
        if ZQAccountsWAccountID.Value = tblSysVarsWCreditorsControlID.Value then
          ...
          // todo : do the sub accounts
          begin
             if LedgerCalculationClass.HasItemOnAccountId(ZQAccountsWAccountID.Value) then
             begin
             LedgerCalculationClass.GetItemOnAccountId(ZQAccountsWAccountID.Value).GetTotalsFromPeriodes(dmDatabase.tblReportingOptions.FieldByName('WTrialBalFromDateID').AsInteger-13,dmDatabase.tblReportingOptions.FieldByName('WTrialBaloDateID').asinteger-13,Debit,Credit) ;
             LYTotal := LYTotal + (debit - credit);
             end;
          end
         end;    // end if trailbalance with uposted batches
    ...
    if ((IncludeOpeningBals) and (ZQAccountsBIncomeExpense.AsInteger = 0)) then begin
      if ZQAccountsWAccountID.Value = dmDatabase.SetOfBooksPropertys.CompanyInfo.RetaintEarnings
       then LYTotal := LYTotal + GetLastYearsOpeningBalance(ZQAccountsWAccountID.AsInteger)
       else LYTotal := LYTotal + GetLastYearsOpeningBalance(ZQAccountsWAccountID.AsInteger);
    end;

    if (ZQAccountsSSUBACCOUNTCODE.Asstring = '000'){ and (tblReportingOptions.FieldByName('BTrialBalDisplaySubAcc').AsBoolean)} and
       (ZQAccountsWAccountID.Asinteger <> dmDatabase.SetOfBooksPropertys.CompanyInfo.DebtorsControlAccount) and (ZQAccountsWAccountID.Asinteger <> dmDatabase.SetOfBooksPropertys.CompanyInfo.CreditorsControlAccount) and
       (ZQAccountsWAccountID.Asinteger <> dmDatabase.SetOfBooksPropertys.CompanyInfo.RetaintEarnings) and (ZQAccountsBSubAccounts.AsInteger =1) or (LYTotal = 0) then
    begin
      //this is a main account
      if not (tblReportingOptions.FieldByName('BTrialBalDisplaySubAcc').AsInteger = 1) then
        if LYTotal > 0 then
    ...
    tblTotals.Filter := '';
    tblTotals.Close;
    if (ZQAccountsBIncomeExpense.Value = 0) then
      IncExpAcc.Caption := ''
    else
      IncExpAcc.Caption := '*';
    //PL
    if (ZQAccountsSSUBACCOUNTCODE.Asstring = '000') {and (tblReportingOptions.FieldByName('BTrialBalDisplaySubAcc').AsBoolean)} and
       (ZQAccountsWAccountID.AsInteger <> tblSysVarsWDebtorsControlID.AsInteger) and (ZQAccountsWAccountID.AsInteger <> tblSysVarsWCreditorsControlID.AsInteger) and
       (ZQAccountsWAccountID.AsInteger <> tblSysVarsWRetainedIncomeID.AsInteger) and ((ZQAccountsBSubAccounts.AsInteger = 1))
       or (not (ZQAccountsBIncomeExpense.AsInteger = 1))  then
      //this is a main account
      begin
      end
    else
 ...
 if (not (dmDatabase.tblReportingOptions.FieldByName('BTrialBalDisplaySubAcc').AsInteger = 1)) and (ZQAccountsSSUBACCOUNTCODE.Asstring <> '000') and (not (ZQAccountsBSubAccounts.AsInteger = 1))then
      PrintBand:=False;
end;
...
begin
  ...
  begin
    ...
    NetProf.left:= (NetProfCr.Left + NetProfCr.Width) - NetProf.Width;
  end
  else
  begin
    ...
    NetProf.left:=  (NetProfDt.Left + NetProfDt.Width) - NetProf.Width;
  end;
  ...
  begin
    ...
    BgNetProf.left:=(BgNetProfCr.Left + BgNetProfCr.Width) - BgNetProf.Width ;
  end
  else
  begin
    ...
    BgNetProf.left:= (BgNetProfDt.Left + BgNetProfDt.Width) - BgNetProf.Width ;
  end;
  ...
  begin
    ...
    LYNetProf.left:=(LYNetProfCr.Left + LYNetProfCr.Width) - LYNetProf.Width ;
  end
  else
  begin
    ...
    LYNetProf.left:=(LYNetProfDt.Left + LYNetProfDt.Width) - LYNetProf.Width ;
  end;
...
begin
  ...
  if CBopeningBalanceOnly then
    Dates.Caption := dmtccorelink.GetTextLang(1066) + ' ' + DateToStr(round(SDate))
  else
...
begin
  ...
  begin
    if dmDatabase.tblGroups.Locate('WGroupID;WGroupTypeID',VarArrayOF([ZQAccountsWReportingGroup1ID.value,20]),[]) then
      Value := GetTextLang(1026){' Total for '}+ dmDatabase.tblGroupsSDescription.value
  end
  else
  if flGroupsID=21 then
  begin
    if dmDatabase.tblGroups.Locate('WGroupID;WGroupTypeID',VarArrayOF([ZQAccountsWReportingGroup2ID.value,21]),[]) then
      Value := GetTextLang(1026){' Total for '}+ dmDatabase.tblGroupsSDescription.value
  end;
