...
uses Main, Database, TrialBalance, Lookup, HtmlHelpAPI, LanguageChoice,
  GlobalFunctions, PrintOptions, TcVariables, DatabaseAccess,XRoutines;
...
//period 1 abd the if opening balances must be included
  ...
  begin
    ...
    qrTrialBalance.QRGroup1.Expression:='QThisYear.WReportingGroup1ID';
    flGroupsID:=20;
  ...
  begin
    ...
    qrTrialBalance.QRGroup1.Expression:='QThisYear.WReportingGroup2ID';
    flGroupsID:=21;
    qrTrialBalance.QRBand3.Height:=38;
  end;
  if qrTrialBalance.IncludeOpeningBals then
  AndClauseThisYearBuget := 'and ((WPeriodID <= ' + IntToStr(ToID) + ' and  WPeriodID >= ' + IntToStr(FromID) + ' ) or WPeriodId = 0)' 
  else
  AndClauseThisYearBuget := 'and (WPeriodID <= ' + IntToStr(ToID) + ' and  WPeriodID >= ' + IntToStr(FromID) + ')' ;

  if fmTrialBalanceOptions.CBopeningBalanceOnly.Checked then
  AndClauseThisYearBuget := 'and (WPeriodId = 0)' ;


  AndClauseLastYear := 'and ((WPeriodID <= ' + IntToStr(ToID-13) + ' and  WPeriodID >= ' + IntToStr(FromID-13) + ' ) ) and BActual = 1' ;

  if fmTrialBalanceOptions.CBopeningBalanceOnly.Checked then
  // dummy select
  AndClauseLastYear := 'and (WPeriodId = -9999)' ;

  qrTrialBalance.QThisYear.close ;
  qrTrialBalance.QLastyear.close ;
  qrTrialBalance.QBuget.close ;
  qrTrialBalance.QThisYearSACCOUNTCODE.Size := MainAccountLength + 3 ;
  qrTrialBalance.QThisYear.SQL.Text := format(dmDatabase.SQLList.GetFormatedSQLByName('Trailbalance_Totals'),[AndClauseThisYearBuget+' and BActual = 1 ',OrderSql]);
 ...
 qrTrialBalance.QThisYear.open ;
  qrTrialBalance.QThisYear.Filtered := False;
...
procedure TfmTrialBalanceOptions.FormCreate(Sender: TObject);
begin
