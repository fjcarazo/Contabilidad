...
//11/01/00
  ...
  begin
    ...
    // This was reported by a Dutch user Jurrian and he search the CVS for the problem
...
procedure TqrBatchTypeReport.QRSysData1Print(sender: TObject;
  var Value: String);
begin
...
procedure TqrBatchTypeReport.QRDBText3Print(sender: TObject;
  var Value: String);
begin
  if dmDatabase.tblMessage.locate('WMessageID',Value,[]) then
    Value := dmDatabase.tblMessage.FieldByName('SDescription').AsString
  else
   Value:='Missing '+ Value;
end;
...
procedure TqrBatchTypeReport.QRDBText5Print(sender: TObject;
  var Value: String);
begin
  if   dmDatabase.tblBatchTypes.Locate('WBatchTypeID',Value,[]) then
    Value:=dmDatabase.tblBatchTypesSDescription.Value
  else
    Value:='';
end;
