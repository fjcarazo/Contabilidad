...
procedure TqrDebtorAgeAnalysis.Foot1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if ((Abs(CurrentTotal + _30daysTotal + _60daysTotal + _90daysTotal)< Abs(dmDatabase.tblReportingOptions.FieldByName('FDebtorAgeMinimumAmount').AsFloat)) AND (dmDatabase.tblReportingOptions.FieldByName('FDebtorAgeMinimumAmount').AsFloat<>0))
     Or
     ((dmDatabase.tblReportingOptions.FieldByName('BDebtorAgeIgnoreZeroBal').Asinteger = 1) AND (FixAmount(CurrentTotal + _30daysTotal + _60daysTotal + _90daysTotal)=0))
      then begin
   PrintBand:=False;
   exit;
  end;
 ...
 // if Abs(CurrentTotal + _30daysTotal + _60daysTotal + _90daysTotal)<0.001 then PrintBand:=False;
  ...
  qrlTotal.Caption := FormatFloat(getviewmask(0),CurrentTotal + _30daysTotal + _60daysTotal + _90daysTotal );
...
//  dmDatabase.tblOilinks.Filter:='WAmountTransactionID='+dmDatabase.qryDebtAgeWTransactionID.AsString;
  dmDatabase.tblOilinks.Filter:='( (WAmountTransactionID='''+dmDatabase.qryDebtAgeWTransactionID.AsString+''') OR (WLinkTransactionID=''' + dmDatabase.qryDebtAgeWTransactionID.AsString+''')) AND DDate>'''+ FormatDateTime('yyyy-mm-dd',fmDebtorAgeAnalysisOptions.wwDBDateTimePicker30Days.Date)+'''';
...
begin
  ...
  _CurrentGrpTotal :=  _CurrentGrpTotal + CurrentTotal;
  ...
  begin
   ...
   qrlTotalg.Caption := FormatFloat(getviewmask(0),CurrentTotal + _30daysTotal + _60daysTotal + _90daysTotal );
  end;
  if (dmDatabase.tblReportingOptions.FieldByName('BDebtorAgeAccountDetail').Asinteger = 1) then
  begin
...
//  TmpFAmt:= dmdatabase.tblOILinksWLinkedAmount.Value-dmdatabase.tblOILinksWOutstandingAmount.Value;
  TmpFAmt:= dmdatabase.tblOILinksWLinkedAmount.Value;
...
begin
  ...
  qrlGrpTotal.Caption := FormatFloat(getviewmask(0),_CurrentGrpTotal + _30daysGrpTotal + _60daysGrpTotal + _90daysGrpTotal );
end;
