...
procedure TqrLedger.QRGroup1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  MvtID:=FromID;
  SkipBand := false;
  ...
  begin
    AccStartBal := GetAccountTotal(dmDatabase.tblAccountWAccountID.Value, FromID, 3);
    AccEndBal := GetAccountTotal(dmDatabase.tblAccountWAccountID.Value, ToID, 3);
  end
  else
  begin
    AccStartBal := GetAccountTotal(dmDatabase.tblAccountWAccountID.Value, FromID, 4);
  ...
  begin
    BeginDebit.Caption := '';
    BeginCredit.Caption := FormatFloat(getviewmask(0),CumulBal * - 1 );
  end;
...
begin
  ...
  if (dmDatabase.tblAccountSSUBACCOUNTCODE.Value='000') AND (dmDatabase.tblAccountBSubAccounts.Value=1) then
    CumulBal:=GetAccountTotal(dmDatabase.tblAccountWAccountID.Value, ToID+1, 3);
  ...
  begin
    EndDebit.Caption := '';
    EndCredit.Caption := FloatToStrTD2C(CumulBal* - 1, ffFixed,  18, 2); 
  end;
...
begin
  ...
  begin
    ...
    qrPrEndCredit.Caption := FloatToStrTD2C(CumulBal * - 1 , ffFixed,  18, 2);
  end;
end;
...
begin
  ...
  Description.Caption:='*** '+ dmDatabase.qryTransactionReportWDescriptionID.AsString+' Missing Description ' ;
  if dmDatabase.tblMessage.Locate('WMessageID', dmDatabase.qryTransactionReportWDescriptionID.Value, []) then
    Description.Caption := dmDatabase.tblMessageSDescription.Value;
  ...
  begin
    Debit.Caption := '';
    Credit.Caption := FloatToStrTD2C(dmDatabase.qryTransactionReportFAmount.Value * - 1, ffFixed, 18, 2);
  end;
