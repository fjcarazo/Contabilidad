...
Var
 i:integer;
 vThis,vLast:Real;
 function GetSql (aAccount,aperiod,aActual : Integer): String ;
 begin
   result := Format('select FAmount from totals where WYearid in (1,2) and WACCOUNTID=%s and WPERIODID = %s and BACTUAL=%s',[IntToStr(aAccount),IntToStr(aperiod),IntToStr(aActual)])
 end;
begin
  //initialise totals
  tempAVarAmnt := 0;
  ...
  // Sylvain
  // Opening Balance
  tempAThisYear := StrToFloatDef(VarToStr(OpenSqlAndGetFirtsColumnValue(GetSql(dmDatabase.tblAccountWAccountID.Value,0, 1))),0);
  qrTotalOp.Caption:=FloatToStrT(tempAThisYear, ffFixed, 18, 2);

  tempALastYear := StrToFloatDef(VarToStr(OpenSqlAndGetFirtsColumnValue(GetSql(dmDatabase.tblAccountWAccountID.Value,0, 0))),0);
  ...
  For i:=1 to 13 do begin
    ...
    vThis := StrToFloatDef(VarToStr(OpenSqlAndGetFirtsColumnValue(GetSql(dmDatabase.tblAccountWAccountID.Value,13+i, 1))),0);
    ...
    if idDocType=255 then begin
      // Budget Total Amount
     vLast := StrToFloatDef(VarToStr(OpenSqlAndGetFirtsColumnValue(GetSql(dmDatabase.tblAccountWAccountID.Value,13+i, 0))),0);
      With qrAccountsMovements.FindComponent('Budget'+IntToStr(i)) as TqrLabel  do
        Caption:=FloatToStrT(vLast, ffFixed, 18, 2);
    end
    else
    begin
     vLast := StrToFloatDef(VarToStr(OpenSqlAndGetFirtsColumnValue(GetSql(dmDatabase.tblAccountWAccountID.Value,i, 0))),0);
...
procedure TqrAccountsMovements.QRSysData1Print(sender: TObject;
  var Value: String);
begin
