...
begin
  ...
  begin
    ...
    dmDatabase.tblOilinks.Filter:='( (WAmountTransactionID='''+dmDatabase.qryDebtStatementWTransactionID.AsString+''') OR (WLinkTransactionID=''' + dmDatabase.qryDebtStatementWTransactionID.AsString+''')) AND DDate>'''+ DateToStr(fmDebtorStatementOptions.wwDBDateTimePicker30Days.Date)+'''';
  ...
  if ((dmdatabase.tblOILinksWAmountTransactionID.AsInteger <> 0) and (dmdatabase.tblOILinksWLinkTransactionID.AsInteger <> 0)) then
     begin
     PrintBand := false ;
     exit ;
     end;
...
//  qrlDateUntil.Caption := dmDatabase.tblReportingOptionsDDebtStatementDateLimit1.AsString;
  ...
  TotBBF := dmDatabase.tblTmpAgeAnalFBBF.Value;
  aMin:=Str2Float(fmDebtorStatementOptions.dbedtMinAmount.Field.AsString);
  if aMin<>0 then
    if ABS(FixAmount(CurrentTotal+_30daysTotal+_60daysTotal+_90daysTotal))<Abs(aMin) then
  begin
    PrintBand:=False;
    Exit;
  end;
...
//  qrlDateUntil2.Caption := dmDatabase.tblReportingOptionsDDebtStatementDateLimit1.AsString;
  ...
  QrVatNbr.Caption := GetTaxRef(dmDatabase.qryDebtStatementWAccountID.AsInteger);
//
...
//  TotBBF:=GetExactAccountToTal(_AccountID,GetPeriod(Date2),Date2);
  ...
  qrlDueAmount.Caption := FloatToStrT(CurrentTotal + _30daysTotal + _60daysTotal + _90daysTotal, ffFixed, 18, -1);
...
//  end
//  else //BBF
    TotCar :=_30daysTotal+_60daysTotal+_90daysTotal +CurrentTotal;
//    TotCar:=CurrentTotal;
  qrCarCr.Caption:='';
  qrCarDT.Caption:='';
  qrBBFDT.Caption:='';
...
//  begin
  ...
  //    IsAmtIncludeTax(TmpFAmt,TxRate,tmpTax,False);
      If tmpTax<0 then
        ...
        qrOiDebit.Caption := FloatToStrT(tmpTax, ffFixed, 18, -1);
    end;
  end
  else
  begin
...
//      dmDatabase.tblMessage.Open;
      ...
      if dmdatabase.tblOILinksWAmountTransactionID.AsString<>dmDatabase.qryDebtStatementWTransactionID.AsString then
        dmDatabase.tblTransaction.Locate('WTransactionID',dmdatabase.tblOILinksWAmountTransactionID.Value,[])
      else
