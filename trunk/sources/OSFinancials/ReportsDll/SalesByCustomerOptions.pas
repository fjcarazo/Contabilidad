...
// 17/07/00
     ...
     _qty     := FloatToStr(Str2Float(_qty)  + FieldByName('FQty').AsInteger);
    end;
   Until Eof;
//   Next;
//  end;
...
begin
  ...
  dmDatabase.qrySalesByCustomer.Filter := 'DDate >= ''' + formatDateTime('yyyy-mm-dd',dmDatabase.tblReportingOptions.FieldByName('DSalesByCustFromDate').AsDateTime) + ''' AND DDate <= ''' + formatDateTime('yyyy-mm-dd',dmDatabase.tblReportingOptions.FieldByName('DSalesByCustToDate').AsDateTime) + '''';
  if BUseCustomer then
    dmDatabase.qrySalesByCustomer.Filter := dmDatabase.qrySalesByCustomer.Filter +
     ' AND (WAccountTypeID = 1)'
  else
    dmDatabase.qrySalesByCustomer.Filter := dmDatabase.qrySalesByCustomer.Filter+
     ' AND (WAccountTypeID = 2)';
  dmDatabase.qrySalesByCustomer.Filtered := True;
...
StockTrn.WStockTransactionID, SDocNo, StockTrn.DDate,
StockTrn.FSellingPrice, StockTrn.FCostPrice, StockTrn.FQty
FROM Account, StockTrn, DocHead
WHERE (SMAINACCOUNTCODE >= '0AC')
AND (SMAINACCOUNTCODE <= '0AT')
AND ((Stocktrn.WDocTypeID = 10) OR (StockTrn.StockWDocTypeID = 11))
AND (Account.WAccountID = StockTrn.WAccountID)
AND (DocHead.WDocID = StockTrn.WDocID)
ORDER BY SMAINACCOUNTCODE, StockTrn.DDate;
