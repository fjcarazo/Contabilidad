...
uses
  ...
  VpMisc,VpSRVars,VpTCEvntEditDlg;

type
  { forward declarations }
  TTCVpNotificationDialog = class;

  TTCAlarmNotifyForm = class(TForm)
    DismissBtn: TButton;
    SnoozeBtn: TButton;
    OpenItemBtn: TButton;
    Bevel1: TBevel;
  ...
  TTCVpNotificationDialog = class(TVpBaseDialog)
  protected {private}
    FBGColor   : TColor;
    ceEditDlg  : TTCAlarmNotifyForm;
    ceTask     : TVpTask;
  public
...
constructor TTCVpNotificationDialog.Create(AOwner : TComponent);
begin
...
procedure TTCVpNotificationDialog.Execute(Event: TVpEvent);
var
  AlarmNotifyForm: TTCAlarmNotifyForm;
begin
  if (Event <> nil) and (not Event.AlertDisplayed) then begin
    Application.CreateForm(TTCAlarmNotifyForm, AlarmNotifyForm);
    try
...
procedure TTCAlarmNotifyForm.PopulateSelf;
begin
...
procedure TTCAlarmNotifyForm.SnoozeComboChange(Sender: TObject);
begin
...
procedure TTCAlarmNotifyForm.SnoozeBtnClick(Sender: TObject);
begin
  CalcSnooze;
  Close;
end;
...
procedure TTCAlarmNotifyForm.DismissBtnClick(Sender: TObject);
begin
...
procedure TTCAlarmNotifyForm.OpenItemBtnClick(Sender: TObject);
begin
  Self.Hide;
  VpEventEditDialog1.DataStore := DataStore;
  VpEventEditDialog1.Execute(Event);
  Close;
end;
{=====}

procedure TTCAlarmNotifyForm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TTCAlarmNotifyForm.CalcSnooze;
begin
  Event.SnoozeTime := Now + SnoozeDelay;
end;
{=====}
procedure TTCAlarmNotifyForm.FormShow(Sender: TObject);
begin
  OpenItemBtn.SetFocus;
end;

procedure TTCAlarmNotifyForm.FormCreate(Sender: TObject);
begin
...
procedure TTCAlarmNotifyForm.FormDestroy(Sender: TObject);
begin
